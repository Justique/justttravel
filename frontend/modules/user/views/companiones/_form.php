...
<?php $form = ActiveForm::begin(['options'=>['class'=>'make-order-form create-req-form']]); ?>
    ...
    <div class="container nutrition-prefs">
        <input style="display: none" type="radio" name="Companiones[gender_traveler]" value="2" id="order-women" <?= $model->gender_traveler == 2 ? 'checked' : '' ?>><label for="order-women">ищу женщину</label>
        <input style="display: none" type="radio" name="Companiones[gender_traveler]" value="3" id="order-man" <?= $model->gender_traveler == 3 ? 'checked' : '' ?>><label for="order-man">ищу мужчину</label>
        <input style="display: none" type="radio" name="Companiones[gender_traveler]" value="1" id="order-company" <?= $model->gender_traveler == 1 ? 'checked' : '' ?>><label for="order-company">ищу компанию</label>
    </div>
...
<div style="" class="companions-container">
    <?php foreach ($myCompaniones as $item): ?>
        ...
        <div class="pull-right">
            <?= Html::a('Изменить', ['update', 'id' => $item->id], ['class' => 'tag']) ?>
            <?= Html::a('<i class="fa fa-trash"></i> Удалить',
                ['delete', 'id' => $item->id],
                ['class' => 'tag', 'data-confirm' => 'Вы уверены, что хотите удалить эту заявку?', 'data-method'=>'post'])
            ?>
        </div>
    </article>
    <?php endforeach; ?>
