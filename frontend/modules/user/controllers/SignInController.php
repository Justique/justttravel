...
class SignInController extends \yii\web\Controller
{
    ...
    public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => ['signup', 'login', 'request-password-reset', 'reset-password', 'oauth','email-confirm'],
                        'allow' => true,
                        'roles' => ['?']
                    ],
                    [
    ...
    public function actionLogin()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->login()) {
            return $this->redirect(url('profile/index'));
        } else {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($user && Yii::$app->getUser()->login($user)) {
                Yii::$app->commandBus->handle(new SendEmailCommand([
                    'from' => [Yii::$app->params['robotEmail'] => 'Justtravel.by'],
                    'to' => $user->email,
                ...
                return $this->render('verifiedEmailConfirm',[
                    'result'=> 'warning',
                ]);
            }
        }
